  ...
  handleFile(event) {
    ...
    if(files.length > 0){
      ...
      if((file.size/1048576)<=4){
        ...
        Swal.fire('Error al importar al archivo excede o limite de tamanho permitido, intente de nuevo!', 'error')
      }
    }
  ...
  sendfile(){
    ...
    this.productoService.importProducto(this.file_data).then( (res:any) =>{    
      if(res.response == true){
        ...
        Swal.fire('Erro al importar los datos de los Producto, intente de nuevo!', 'error')
      }
    }).catch(err=>{
      console.log(err);
    });
